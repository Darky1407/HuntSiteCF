...
async function onRequestPost(context2) {
  const { answer } = await context2.request.json();
  const CORRECT_ANSWER = context2.env.FINAL_ANSWER;
...
async function onRequestPost2(context2) {
  const { username, stage } = await context2.request.json();
  const cleaned = username.trim().toLowerCase().replace(/\s+/g, "_");
  const projectId = context2.env.FIREBASE_PROJECT_ID;
  const token = await getFirebaseAccessToken(context2.env);
  ...
  const complete = doc.fields?.[stage]?.booleanValue === true;
  return new Response(JSON.stringify({ complete }), {
    headers: { "Content-Type": "application/json" }
  });
}
__name(onRequestPost2, "onRequestPost");
async function getFirebaseAccessToken(env2) {
  const header = { alg: "RS256", typ: "JWT" };
  const now = Math.floor(Date.now() / 1e3);
  const claim = {
    iss: env2.FIREBASE_CLIENT_EMAIL,
  ...
  const key = await crypto.subtle.importKey(
    "pkcs8",
    pemToBuffer(env2.FIREBASE_PRIVATE_KEY),
...
async function onRequestPost3(context2) {
  const { answer } = await context2.request.json();
  const CORRECT_ANSWER = context2.env.STAGE1_ANSWER;
  const normalized = answer.trim().toLowerCase().replace(/\s+/g, " ");
  const isCorrect = normalized === CORRECT_ANSWER;
  return new Response(JSON.stringify({ correct: isCorrect }), {
    headers: { "Content-Type": "application/json" }
...
async function onRequestPost4(context2) {
  const { answer } = await context2.request.json();
  const CORRECT_ANSWER = context2.env.STAGE2_ANSWER;
  const normalized = answer.trim().toLowerCase().replace(/\s+/g, " ");
  const isCorrect = normalized === CORRECT_ANSWER;
  return new Response(JSON.stringify({ correct: isCorrect }), {
    headers: { "Content-Type": "application/json" }
...
async function onRequestPost5(context2) {
  const { answer } = await context2.request.json();
  const CORRECT_ANSWER = context2.env.STAGE3_ANSWER;
  const normalized = answer.trim().toLowerCase().replace(/\s+/g, " ");
  const isCorrect = normalized === CORRECT_ANSWER;
  return new Response(JSON.stringify({ correct: isCorrect }), {
    headers: { "Content-Type": "application/json" }
...
async function onRequestPost6(context2) {
  const { answer } = await context2.request.json();
  const CORRECT_ANSWER = context2.env.STAGE4_ANSWER;
  const normalized = answer.trim().toLowerCase().replace(/\s+/g, " ");
  const isCorrect = normalized === CORRECT_ANSWER;
  return new Response(JSON.stringify({ correct: isCorrect }), {
    headers: { "Content-Type": "application/json" }
...
async function onRequestPost7(context2) {
  const { username } = await context2.request.json();
  const cleaned = username.trim().toLowerCase().replace(/\s+/g, "_");
  const projectId = context2.env.FIREBASE_PROJECT_ID;
  const token = await getFirebaseAccessToken2(context2.env);
  const res = await fetch(
    `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/sessions/${cleaned}`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
...
async function getFirebaseAccessToken2(env2) {
  const header = { alg: "RS256", typ: "JWT" };
  const now = Math.floor(Date.now() / 1e3);
  const claim = {
    iss: env2.FIREBASE_CLIENT_EMAIL,
  ...
  const key = await crypto.subtle.importKey(
    "pkcs8",
    pemToBuffer2(env2.FIREBASE_PRIVATE_KEY),
    { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" },
    false,
    ["sign"]
  );
...
__name(getFirebaseAccessToken2, "getFirebaseAccessToken");
function pemToBuffer2(pem) {
  const b64 = pem.replace(/-----[^-]+-----/g, "").replace(/\s/g, "");
  const binary = atob(b64);
  const buffer = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i++) buffer[i] = binary.charCodeAt(i);
...
__name(pemToBuffer2, "pemToBuffer");
...
async function onRequestPost8(context2) {
  ...
  const cleaned = username.trim().toLowerCase().replace(/\s+/g, "_");
  const projectId = context2.env.FIREBASE_PROJECT_ID;
  const token = await getFirebaseAccessToken3(context2.env);
  await fetch(
    `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/sessions/${cleaned}`,
    {
      method: "PATCH",
...
async function getFirebaseAccessToken3(env2) {
  const header = { alg: "RS256", typ: "JWT" };
  const now = Math.floor(Date.now() / 1e3);
  const claim = {
    iss: env2.FIREBASE_CLIENT_EMAIL,
  ...
  const key = await crypto.subtle.importKey(
    "pkcs8",
    pemToBuffer3(env2.FIREBASE_PRIVATE_KEY),
    { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" },
    false,
    ["sign"]
  );
...
__name(getFirebaseAccessToken3, "getFirebaseAccessToken");
function pemToBuffer3(pem) {
  const b64 = pem.replace(/-----[^-]+-----/g, "").replace(/\s/g, "");
  const binary = atob(b64);
  const buffer = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i++) buffer[i] = binary.charCodeAt(i);
...
__name(pemToBuffer3, "pemToBuffer");

// notify-complete.js
async function onRequestPost9(context2) {
  const { name, time: time3 } = await context2.request.json();
  const WEBHOOK_URL = context2.env.DISCORD_WEBHOOK_URL;
  await fetch(WEBHOOK_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      content: `\u{1F389} ${name} reached the landing page at ${time3}`
    })
  });
  return new Response(JSON.stringify({ ok: true }), {
    headers: { "Content-Type": "application/json" }
...
// ../.wrangler/tmp/pages-rLixlq/functionsRoutes-0.3049166431565892.mjs
var routes = [
  {
